...
#Please note: I don't know why I can't get this to render to pdf, so I submitted it as a qmd. Sorry! I'll to to figure it out for next time!
...
# Plotting
ggplot(top5_CA, aes(x = as.factor(Year), y = Value, fill = chem_name)) +
...
#base on the results, I want to investigate sulfur


```

```{r}
# I want to get a dataframe containing all rows with chem_name = sulfur

chem_ca_sulfur <- chem_ca |> filter(chem_ca$chem_name == "SULFUR")

#impute the missing value for LB in 2023, which i noticed was missing from the graph because I originally
#took out NAs but seeing how it has other values for 2023 with other units like lb/acre/yr and lb/acre/app
#it was likely used in 2023, so I will impute the missing data 
...
# Conclusion of Sulfur: based on what I have seen with sulfur, I am incline to believe that other chemicals that didn't show up in the top 5 for all the years but showed up in some of the years might be missing and needs imputation. Will look into this....

```{r}
# I want to get a dataframe containing all rows with chem_name = dichloropropene
...
# I'm not sure why the lb/acre/year increased so much between 2018 and 2019 while the lb/acre/application remained fairly the same...

#Let's look at how many of the top 5 chemicals in CA contains hazards that are "fatal". I skipped Dichloropropene and Neem Oil because I couldn't readily obtain it from the GHS Search. So I will only do the 9 chemicals that showed up in the top 5 for year 2018, 2019, 2021, 2023.
...
### Sorry I ran out of time, but with more time, I would look into other hazards such as carcinogenic, skin irritants and make a graph of the 9 chemicals showing of each chemicals, how many hazards it contains.

### I would also re-graph the top-5 CA with the imputed values to see what it is "predicted" to look like vs. with raw missing data...and leave it to the audience to decide which picture is more accurate
...
### Florida seems to really only have a "top 2" instead of "top 5". And they happen to be Captan and Thiram, the chemicals that contain "fatal" hazards in the California top 5!!!! Let's plot the two chemicals values for florida and california to compare.
...
### Florida seems to use a more close proportion of Captan and Thiram whereas California appers to prefer Captan.. I wonder why that is. I would have to dig into the other hazards that these two chemicals contain and try to think of why California chose one over the other.
